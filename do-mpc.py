...
rx=20
ry=10
elli=patches.Ellipse(xy=xo,width=rx, height=ry, angle=0)
ax.add_artist(elli)
x=[0,0,0]
...
#    plt.pause(0.5)
    ...
    Y_ref=[60, 60, 1.57]
    V=(lam*(rx**4)*(ry**4))/(gamma*(rx**2)*(ry**2)+(ry**2)*(xo[0]-x[0])**2+(rx**2)*(xo[1]-x[1])**2)**2
    print "cost", V
    print "x cost:",(x[0]-Y_ref[0])**2,"y cost:",(x[1]-Y_ref[1])**2,"theta cost:", (x[2]-Y_ref[2])**2
    a2=ax.arrow(x[0],x[1],3*math.cos(x[2]),3*math.sin(x[2]), head_width=0.08, head_length=0.00002)
    plt.pause(0.1)
    """
    ----------------------------
